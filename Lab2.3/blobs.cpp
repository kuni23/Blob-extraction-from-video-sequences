...
{	
	...
	//Applying the floodfill algortihmm
	 for (int i=0; i<aux.rows;i++)
			 for(int j=0; j<aux.cols;j++)
			 {
				 ...
				 {
					 Rect r;
					 //23 is a random number, it must be not 255!
					 floodFill(aux, Point(j, i), 23, &r, 0, 0, connectivity);
					 cvBlob blob = initBlob(1, r.x, r.y, r.width, r.height);
 ...
 int classifyBlobs(std::vector<cvBlob> &bloblist)
 {
	...
	vector<double> diff;
 	...
 	for(int i = 0; i < bloblist.size(); i++)
 	{
 		...
 		double ratio= (double) blob.w/blob.h;

 		result.clear();
 		diff.clear();


 		//Variables in each cycle
 		bool person=abs(ratio-MEAN_PERSON)<STD_PERSON;
 		bool car = abs(ratio-MEAN_CAR)<STD_CAR;
 		bool object = abs(ratio-MEAN_OBJECT)<STD_OBJECT;

 		if(person) {blob.label=PERSON;}
 		//Problem can happen if car and object are also detected. So the one is closer to the ratio, will be the classified label.
 		if(car & object)
 		{
 			if(abs(ratio-MEAN_CAR)<abs(ratio-MEAN_OBJECT))
 			{
 				blob.label=CAR;
 			}
 			else
 		...
 		{
 	 		if(car) blob.label=CAR;
 	 		if(object) blob.label=OBJECT;
 		}
...
#define SECS_STATIONARY 3 // to set
...
#define STAT_TH 0.0 // to set
 ...
 int extractStationaryFG (Mat fgmask, Mat &fgmask_history, Mat &sfgmask)
 {
	 ...
	 //Background substraction is already performed  (we have the mask)
	 ...
	 //measure FG temporal variation to get FG history img
 	 // update fgmask_counter --> increases for FG, decreases for BG
	 ...
	 for (int i=0; i<fgmask.rows;i++){
		 for(int j=0; j<fgmask.cols;j++){
			 if (fgmask.at<uchar>(i,j)==255){
				 fgmask_history.at<float>(i,j) = fgmask_history.at<float>(i,j)+Wpos; // FHI (t) = FHI (t−1) + w * FG (t)
			 }else{
				 if(fgmask_history.at<float>(i,j)-Wneg < 0){
					 fgmask_history.at<float>(i,j) = 0;
				 }else{
					 fgmask_history.at<float>(i,j) = fgmask_history.at<float>(i,j)-Wneg; // FHI (t) = FHI (t−1) + w * FG (t)
				 }
			 }
		 }
	 }

	 // AT FGMASK_HISTORY WE HAVE A FOREGROUND SCORE, WHICH INCREASES WHEN THE PIXEL IS FG AND DECREASES WHEN IT IS BKG


	 // "combination" and normalization
	 //fgmask_history=min(255,(fgmask_history/numframes4static));

	 cv::Mat fgmask_history_normalized=min(1,(fgmask_history/numframes4static));

	 //update sfgmask
	 for (int i=0; i<fgmask.rows;i++){
		  for(int j=0; j<fgmask.cols;j++){
			  if(fgmask_history_normalized.at<float>(i,j)>=th){
	 ...
	 //imshow("sfgmask",sfgmask);
	 //imshow("norm FHI",fgmask_history_normalized);
